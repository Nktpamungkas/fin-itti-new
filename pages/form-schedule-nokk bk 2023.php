...
<?php 
	if($_POST['save']=="save"){
	  ...
	  $qryCekN=mysqli_query($con,"SELECT * from tbl_schedule WHERE nokk='$_POST[nokk]' and  no_mesin='$_POST[no_mc]' and proses='".$_POST['proses']."' and DATE_FORMAT(tgl_update,'%Y-%m-%d')=DATE_FORMAT( now( ), '%Y-%m-%d' )");
	  $rowN=mysqli_num_rows($qryCekN);
	  $qryCekN1=mysqli_query($con,"SELECT * from tbl_schedule WHERE nokk='$_POST[nokk]' and no_urut='$_POST[no_urut]' and  no_mesin='$_POST[no_mc]' and proses='".$_POST['proses']."' and DATE_FORMAT(tgl_update,'%Y-%m-%d')=DATE_FORMAT( now( ), '%Y-%m-%d' )");
